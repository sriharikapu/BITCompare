...
unsigned int BitcoinGetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    if ((pindexLast->nHeight+1) % params.DifficultyAdjustmentInterval() != 0)
    {
        if (params.fPowAllowMinDifficultyBlocks)
        {
            ...
            if (pindexLast->nHeight < 498533) {
                if (pblock->GetBlockTime() > pindexLast->GetBlockTime() + params.nPowTargetSpacingLegacy*2)
                    return nProofOfWorkLimit;
                else
                {
                ...
                if (pblock->GetBlockTime() > pindexLast->GetBlockTime() + params.nPowTargetBlockTime*2)
                    return nProofOfWorkLimit;
                else
                {
                    // Return the last non-special-min-difficulty-rules-block
