...
bool CanDirectFetch(const Consensus::Params &consensusParams)
{
    ...
    if (pindex->nHeight < 498533) {
        return chainActive.Tip()->GetBlockTime() > target_time - consensusParams.nPowTargetSpacingLegacy * 20;
    } else {
        return chainActive.Tip()->GetBlockTime() > target_time - consensusParams.nPowTargetBlockTime * 20;        
    }
}
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman& connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    else if (strCommand == NetMsgType::GETBLOCKS)
    {
        ...
        for (; pindex; pindex = chainActive.Next(pindex))
        {
            ...
            int nPrunedBlocksLikelyToHave;
            if (pindex->nHeight < 498533) {
                nPrunedBlocksLikelyToHave = MIN_BLOCKS_TO_KEEP - 3600 / chainparams.GetConsensus().nPowTargetSpacingLegacy;
            } else {
                nPrunedBlocksLikelyToHave = MIN_BLOCKS_TO_KEEP - 3600 / chainparams.GetConsensus().nPowTargetBlockTime;
            }
    ...
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (!state.fSyncStarted && !pto->fClient && !fImporting && !fReindex) {
            ...
            if ((nSyncStarted == 0 && fFetch) || pindexBestHeader->GetBlockTime() > GetAdjustedTime() - 24 * 60 * 60) {
                ...
                if (pindexBestHeader->nHeight < 498533) {
                    state.nHeadersSyncTimeout = GetTimeMicros() + HEADERS_DOWNLOAD_TIMEOUT_BASE + HEADERS_DOWNLOAD_TIMEOUT_PER_HEADER * (GetAdjustedTime() - pindexBestHeader->GetBlockTime())/(consensusParams.nPowTargetSpacingLegacy);
                } else {
                    state.nHeadersSyncTimeout = GetTimeMicros() + HEADERS_DOWNLOAD_TIMEOUT_BASE + HEADERS_DOWNLOAD_TIMEOUT_PER_HEADER * (GetAdjustedTime() - pindexBestHeader->GetBlockTime())/(consensusParams.nPowTargetBlockTime);
                }
                nSyncStarted++;
        ...
        if (state.vBlocksInFlight.size() > 0) {
            ...
            if (pindexBestHeader->nHeight < 498533) {  
                if (nNow > state.nDownloadingSince + consensusParams.nPowTargetSpacingLegacy * (BLOCK_DOWNLOAD_TIMEOUT_BASE + BLOCK_DOWNLOAD_TIMEOUT_PER_PEER * nOtherPeersWithValidatedDownloads)) {
                    LogPrintf("Timeout downloading block %s from peer=%d, disconnecting\n", queuedBlock.hash.ToString(), pto->GetId());
                    pto->fDisconnect = true;
                    return true;
                }
            } else { 
                if (nNow > state.nDownloadingSince + consensusParams.nPowTargetBlockTime * (BLOCK_DOWNLOAD_TIMEOUT_BASE + BLOCK_DOWNLOAD_TIMEOUT_PER_PEER * nOtherPeersWithValidatedDownloads)) {
                    LogPrintf("Timeout downloading block %s from peer=%d, disconnecting\n", queuedBlock.hash.ToString(), pto->GetId());
                    pto->fDisconnect = true;
                    return true;
                }  
